...
<%  English_String=""
	...
	<%
	Limit=0
	 rs.open "Select count(*) from tab_assentences", Conn
		if rs.eof Then
			'pass
		else
	 ...
	 rs.close
	 rs.open "select * from tab_assentences ORDER BY Assamese", Conn 
			if rs.eof Then
				'Do Search
				
			else
			     Response.write ("<h1 align=center>Assamese to English Translation</h1>")
				 Response.write ("<table name=tab id=assamesetable border=1 align=center>")
				 Response.write ("<tr>")
				 Response.Write ("<th>অসমীয়া বাক্য</th>")
				 Response.Write ("<th>ঈংৰাজী অনুবাদ</th>")
				 Response.Write ("<th>ঈংৰাজী উচ্চাৰণ</th>")
				  Response.Write ("<th>শুনা</th>")
				 Response.Write ("</tr>")
	...
	<script>
	 ...
	 {
	   utter.voice=speechSynthesis.getVoices()[9]  ;
	   x=document.getElementById("assamesetable").rows[count+1].cells[1].innerHTML;
	   EngStrUtchnew=x;
	   txt="";
	   txt=x;
	   utter.text=txt;
